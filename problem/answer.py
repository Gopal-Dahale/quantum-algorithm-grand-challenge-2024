...
import math
from initial_state_prep import get_initial_state_circuit
...
class ADAPT_QSCI:
    def __init__(
        ...
        atol: float = 1e-5,
        round_op_config: Union[dict, Tuple[Optional[int], Optional[float]]] = (
            None,
            1e-2,
        ),
        ...
        self.initial_state_circ = initial_state_circ
        self.comp_basis = [hf_state]
        # gradient
        if round_op_config is None:
            round_op_config = (None, None)
        num_pickup: int = (
            round_op_config["num_pickup"]
            if isinstance(round_op_config, dict)
            else round_op_config[0]
        )
        coeff_cutoff: float = (
            round_op_config["cutoff"]
            if isinstance(round_op_config, dict)
            else round_op_config[1]
        )
        self.num_pickup = num_pickup
        self.coeff_cutoff = coeff_cutoff
        round_ham = round_hamiltonian(
            hamiltonian, num_pickup=num_pickup, coeff_cutoff=coeff_cutoff
        )
        ...
        self.num_precise_gradient: int = (
            len(pool) if num_precise_gradient is None else num_precise_gradient
        )
        self.pauli_rotation_circuit_qsci = PauliRotationCircuit(
            [], [], [], n_qubits + num_ancillas
        )
        self.num_ancillas = num_ancillas
        self.ignored_gen_inx = []
        self.reset_ignored_inx_mode: int = (
            reset_ignored_inx_mode if reset_ignored_inx_mode > 0 else iter_max
        )
        # convergence
        assert max_num_converged >= 1
        self.final_sampling_shots: int = int(
            final_sampling_shots_coeff * sampling_shots
        )
    ...
    def _get_optimized_parameter(
        ...
        exp_commutator = (vec_qsci.T.conj() @ commutator_sparse @ vec_qsci).item().real
    ...
    def run(self) -> float:
        ...
        for itr in range(1, self.iter_max + 1):
            ...
            if self.num_precise_gradient is not None and self._is_grad_round:
                ...
                sorted_keys = sorted(
                    precise_grad_vals.keys(),
                    key=lambda x: abs(precise_grad_vals[x]),
                    reverse=True,
                )
                ...
                if self.check_duplicate:
                    if (
                        len(self.operator_index_history) >= 1 and len(sorted_keys) >= 2
                    ) and (sorted_keys[0] == self.operator_index_history[-1]):
            ...
            new_coeff, new_pauli_str = (
                float(operator_coeff_term[0]),
                operator_coeff_term[1],
            )
            self.generator_history.append(new_pauli_str)

            # add new generator to ansatz
            new_param_name = f"theta_{itr}"
            circuit_qsci = self.pauli_rotation_circuit_qsci.add_new_gates(
                new_pauli_str, new_coeff, new_param_name
            )
            new_param_value = self._get_optimized_parameter(vec_qsci, self.comp_basis)
            if np.isclose(new_param_value, 0.0):
            ...
            try:
                ...
                print(
                    f"ansatz seems to have no gates since optimized parameter was {new_param_value}"
                )
                raise

            # increase sampling shots when same generator is selected twice in a row or parameter is close to 0.
            is_alert = (
                new_gen_indices in self.generator_qubit_indices_history
                or np.isclose(0.0, new_param_value)
            )
            self.generator_qubit_indices_history.append(new_gen_indices)
            sampling_shots = (
                self.final_sampling_shots if is_alert else self.sampling_shots
            )

            # prepare circuit for QSCI
            parametric_state_qsci = prepare_parametric_state(
                self.initial_state_circ, circuit_qsci
            )
            target_circuit = parametric_state_qsci.parametric_circuit.bind_parameters(
                self.param_values
            )
            ...
            try:
                ...
                counts = dict(counts)
                counts = {
                    bin(i)[2:].zfill(self.n_qubits + self.num_ancillas): j
                    for i, j in counts.items()
                }
                counts = marginal_counts(counts, list(range(self.n_qubits)))
                counts = Counter({int(i, 2): j for i, j in counts.items()})
            ...
            self.qsci_energy_history.append(val_qsci)
            print(
                f"basis selected: {[bin(b.bits)[2:].zfill(self.n_qubits) for b in self.comp_basis]}"
            )
...
def create_ham_inspired_pool(qubit_hamiltonian, n_qubits) -> list[Operator]:
    pauli_lookup = {"Z": 1, "X": 2, "Y": 3}
    pauli_reverse_lookup = ["I", "Z", "X", "Y"]
    pool_generators, pool_tuples, pool_commutators = list(), list(), list()
    indices = list()
    ...
    for term in qubit_hamiltonian.terms:
        ...
        if sum(pauli_string) % 2 == 0 and sum(pauli_string) > 0:
            ...
            operator_tuple = tuple(
                [
                    (index, pauli_reverse_lookup[pauli])
            ...
            qubit_op = QubitOperator(operator_tuple, 1.0j)

            # Cast all coefs to floats (rotations angles are real).
            for term, coeff in qubit_op.terms.items():
                qubit_op.terms[term] = math.copysign(1.0, coeff.imag)

            pool_generators.append(operator_from_openfermion_op(qubit_op))
...
class RunAlgorithm:
    ...
    def result_for_evaluation(
        self, seed: int, hamiltonian_directory: str
    ) -> tuple[Any, float]:
    ...
    def get_result(self, seed: int, hamiltonian_directory: str) -> float:
        ...
        pool = create_ham_inspired_pool(
            jw_hamiltonian,
            n_qubits,
        )

        initial_state_circ, num_ancillas = get_initial_state_circuit(n_qubits, ham)
